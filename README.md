...
作業ディレクトリ（`/home/node/workspace/`）にスキルをコピーします。

```bash
# スキルをコピーします（何度実行しても OK。既存のスキルは上書き）
mkdir -p /home/node/workspace/.claude/skills && cp -rf /home/node/workspace/2026-claude-code-workshop-assets/.claude/skills/. /home/node/workspace/.claude/skills/
```

### 想定ディレクトリ構成

このリポジトリを `git clone` すると、以下のような配置になります。

```
2026-claude-code-workshop-assets/  ← git clone で取得（このリポジトリ）
├── README.md
└── samples/                       ← サンプルプロジェクト
```
...
2. Claude Code で `/scope-design` を実行
   → プロジェクト名を聞かれるので答える
   → input.md → idea.md → scope.md が作成される
        ↓
3. scope.md を元に実装（apps/ 配下にアプリ本体）
        ↓
4. Claude Code で `/verify-project` を実行
   → verification.md が作成される
        ↓
5. Claude Code で `/study-plan` を実行（メタ認知）
   → study-plan.md が作成される
        ↓
6. ペアフィードバック → ピッチ発表
        ↓
7. （任意）Claude Code で `/archive-project` を実行
   → projects/<名前>/ を ZIP にまとめてダウンロード
```
...
### `/verify-project` — 検証 + 発表準備を対話でサポート

**使いどころ**: 実装が一段落したとき（ピッチ前）

やってくれること：

1. 「まずプロダクトを触って、課題文と照らし合わせてください」と促す
2. ワンライナー / 課題と解決策の振り返り / デモシナリオ / 技術的な根拠 / 限界と次の一手 を1問ずつ聞く
3. ピッチ発表の骨子として使える `verification.md` を書き出し

→ 詳細: [`.claude/skills/verify-project/SKILL.md`](.claude/skills/verify-project/SKILL.md)

### `/study-plan` — 持ち帰りの学習計画を対話で作成

**使いどころ**: `/verify-project` の後、ピッチ発表の前（メタ認知してから登壇）

やってくれること：

1. 「もし実際にデリバリーするとしたら何が足りないか」を一緒に棚卸し
2. 認証・認可 / 入力の扱い / 利用規約 / 個人情報 / コスト / 運用 などを1問ずつ平易な言葉で聞く（専門用語は学生の回答に対して命名で返す）
3. 興味のある 1〜3 領域に絞った学習ロードマップ（入門記事 → 公式ドキュメントの順）を `study-plan.md` に書き出し

→ 詳細: [`.claude/skills/study-plan/SKILL.md`](.claude/skills/study-plan/SKILL.md)

### `/archive-project` — 成果物を ZIP にまとめて持ち帰り

**使いどころ**: ワークショップ終了時、code-server から成果物をダウンロードしたいとき

やってくれること：

1. `projects/<name>/` 配下を一式 ZIP に固める
2. `node_modules` / `.venv` / `__pycache__` など再生成可能な重いファイルは自動で除外
3. ターミナル操作に不慣れでも1回の呼び出しで持ち帰りが完結

→ 詳細: [`.claude/skills/archive-project/SKILL.md`](.claude/skills/archive-project/SKILL.md)
...
```
2026-claude-code-workshop-assets/
├── README.md                          # このファイル
├── .claude/
│   └── skills/
...
開発演習本番では code-server 上に上記が用意されています。ローカルで試す場合は各自で準備してください。
...
このリポジトリは、Claude Code を活用した開発演習向け教材として制作されました。
